...
@Component({
  selector: 'app-rental-transition-cancel',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './rental-transition-cancel.html',
  styleUrl: './rental-transition-page.scss',
})
export class RentalTransitionCancel implements OnInit {
    ...
    this.rentalApiClient
      ...
      .pipe(
        catchError(() => {
          ...
          this.snackBar.open('Failed to cancel the rental. Please try again.', 'Close', { duration: 4000 });
          return EMPTY;
        })
      )
      .subscribe(() => {
        this.snackBar.open('Rental cancelled.', 'Close', { duration: 3000 });
