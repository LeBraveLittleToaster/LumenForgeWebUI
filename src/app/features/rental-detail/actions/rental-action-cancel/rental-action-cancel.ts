...
import { CancelRentalDto, RentalActionView, RentalApiClient, RentalProcessView } from '@lumenforge/api-client';
...
@Component({
  ...
  templateUrl: './rental-action-cancel.html',
  styleUrl: './rental-action-cancel.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class RentalActionCancel implements OnInit {
  ...
  readonly isAvailable = computed(() =>
    this.availableActions().some(a => normalizeActionType(a) === 'cancel')
  );
  ...
  submit(): void {
    ...
    const dto: CancelRentalDto = { reason: this.reason.value.trim() };
    this.submitting.set(true);
    this.rentalApiClient.cancel(this.processGuid, dto).pipe(
      finalize(() => this.submitting.set(false))
    ).subscribe({
      next: () => {
        this.snackBar.open('Rental process cancelled.', 'Close', { duration: 3000 });
