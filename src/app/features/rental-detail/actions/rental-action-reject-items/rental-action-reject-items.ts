...
import { RejectItemsDto, RentalActionView, RentalApiClient, RentalProcessView } from '@lumenforge/api-client';
...
@Component({
  selector: 'app-rental-action-reject-items',
  standalone: true,
  imports: [
  ...
  templateUrl: './rental-action-reject-items.html',
  styleUrl: './rental-action-reject-items.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class RentalActionRejectItems implements OnInit {
  ...
  readonly isAvailable = computed(() =>
    this.availableActions().some(a => normalizeActionType(a) === 'reject-items')
  );
  ...
  submit(): void {
    ...
    const dto: RejectItemsDto = { reason: this.reason.value.trim() };
    this.submitting.set(true);
    this.rentalApiClient.rejectItems(this.processGuid, dto).pipe(
      finalize(() => this.submitting.set(false))
    ).subscribe({
      next: () => {
        this.snackBar.open('Assigned items rejected.', 'Close', { duration: 3000 });
