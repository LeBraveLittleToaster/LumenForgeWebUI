...
import { RecordPickupDto, RentalActionView, RentalApiClient, RentalProcessView } from '@lumenforge/api-client';
...
export class RentalActionRecordPickup implements OnInit {
  ...
  readonly isAvailable = computed(() =>
    this.availableActions().some(a => normalizeActionType(a) === 'record-pickup')
  );
  ...
  submit(): void {
    ...
    this.rentalApiClient.recordPickup(this.processGuid, dto).pipe(
      finalize(() => this.submitting.set(false))
    ).subscribe({
      next: () => {
        this.snackBar.open('Pickup recorded.', 'Close', { duration: 3000 });
