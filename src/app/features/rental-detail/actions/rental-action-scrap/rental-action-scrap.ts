...
import { ScrapRentalDto, RentalActionView, RentalApiClient, RentalProcessView } from '@lumenforge/api-client';
...
@Component({
  ...
  templateUrl: './rental-action-scrap.html',
  styleUrl: './rental-action-scrap.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class RentalActionScrap implements OnInit {
  ...
  readonly isAvailable = computed(() =>
    this.availableActions().some(a => normalizeActionType(a) === 'scrap')
  );
  ...
  submit(): void {
    ...
    if (!this.reason.value.trim()) {
      this.snackBar.open('Scrap reason is required.', 'Close', { duration: 3000 });
      return;
    }
    const dto: ScrapRentalDto = { reason: this.reason.value.trim() };
    this.submitting.set(true);
    this.rentalApiClient.scrap(this.processGuid, dto).pipe(
      finalize(() => this.submitting.set(false))
    ).subscribe({
      next: () => {
        this.snackBar.open('Rental process scrapped.', 'Close', { duration: 3000 });
