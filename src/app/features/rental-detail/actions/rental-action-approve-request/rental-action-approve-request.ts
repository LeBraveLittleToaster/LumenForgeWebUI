...
import { ApproveRequestDto, RentalActionView, RentalApiClient, RentalProcessView } from '@lumenforge/api-client';
...
export class RentalActionApproveRequest implements OnInit {
  ...
  readonly isAvailable = computed(() =>
    this.availableActions().some(a => normalizeActionType(a) === 'approve-request')
  );
  ...
  submit(): void {
    ...
    this.rentalApiClient.approveRequest(this.processGuid, dto).pipe(
      finalize(() => this.submitting.set(false))
    ).subscribe({
      next: () => {
        this.snackBar.open('Rental request approved.', 'Close', { duration: 3000 });
