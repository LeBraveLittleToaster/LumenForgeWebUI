...
import { RentalActionView, RentalApiClient, RentalProcessView, SignChecklistDto } from '@lumenforge/api-client';
...
export class RentalActionSignChecklist implements OnInit {
  ...
  readonly isAvailable = computed(() =>
    this.availableActions().some(a => normalizeActionType(a) === 'sign-checklist')
  );
  ...
  submit(): void {
    ...
    if (!this.signatureData.value.trim()) {
      this.snackBar.open('Signature data is required.', 'Close', { duration: 3000 });
      return;
    }
    const dto: SignChecklistDto = {
      checklist_guid: this.checklistGuid.value,
      signature_data: this.signatureData.value.trim(),
    };
    this.submitting.set(true);
    this.rentalApiClient.signChecklist(this.processGuid, dto).pipe(
      finalize(() => this.submitting.set(false))
    ).subscribe({
      next: () => {
        this.snackBar.open('Checklist signed.', 'Close', { duration: 3000 });
