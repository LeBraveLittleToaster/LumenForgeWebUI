...
import { RentalActionView, RentalApiClient, RentalProcessView } from '@lumenforge/api-client';
...
@Component({
  ...
  templateUrl: './rental-action-complete.html',
  styleUrl: './rental-action-complete.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class RentalActionComplete implements OnInit {
  ...
  readonly isAvailable = computed(() =>
    this.availableActions().some(a => normalizeActionType(a) === 'complete')
  );
  ...
  submit(): void {
    ...
    this.rentalApiClient.complete(this.processGuid, { comment: this.toNullable(this.comment.value) }).pipe(
      finalize(() => this.submitting.set(false))
    ).subscribe({
      next: () => {
        this.snackBar.open('Rental process completed.', 'Close', { duration: 3000 });
