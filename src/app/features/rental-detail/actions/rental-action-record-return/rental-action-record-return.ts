...
import { RecordReturnDto, RentalActionView, RentalApiClient, RentalProcessView } from '@lumenforge/api-client';
...
@Component({
  ...
  templateUrl: './rental-action-record-return.html',
  styleUrl: './rental-action-record-return.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class RentalActionRecordReturn implements OnInit {
  ...
  readonly isAvailable = computed(() =>
    this.availableActions().some(a => normalizeActionType(a) === 'record-return')
  );
  ...
  submit(): void {
    ...
    this.rentalApiClient.recordReturn(this.processGuid, dto).pipe(
      finalize(() => this.submitting.set(false))
    ).subscribe({
      next: () => {
        this.snackBar.open('Return recorded.', 'Close', { duration: 3000 });
