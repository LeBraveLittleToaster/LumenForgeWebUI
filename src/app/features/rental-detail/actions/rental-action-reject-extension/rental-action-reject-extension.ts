...
import { RejectExtensionDto, RentalActionView, RentalApiClient, RentalProcessView } from '@lumenforge/api-client';
...
export class RentalActionRejectExtension implements OnInit {
  ...
  readonly isAvailable = computed(() =>
    this.availableActions().some(a => normalizeActionType(a) === 'reject-extension')
  );
  ...
  submit(): void {
    ...
    this.rentalApiClient.rejectExtension(this.processGuid, dto).pipe(
      finalize(() => this.submitting.set(false))
    ).subscribe({
      next: () => {
        this.snackBar.open('Extension rejected.', 'Close', { duration: 3000 });
