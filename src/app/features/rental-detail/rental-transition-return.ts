...
@Component({
  selector: 'app-rental-transition-return',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './rental-transition-return.html',
  styleUrl: './rental-transition-page.scss',
})
export class RentalTransitionReturn implements OnInit {
  ...
  onSubmit(): void {
    ...
    const notes = this.form.value.conditionNotes?.trim() || null;
    ...
    this.rentalApiClient
      ...
      .pipe(
        catchError(() => {
          ...
          this.snackBar.open('Failed to record return. Please try again.', 'Close', { duration: 4000 });
          return EMPTY;
        })
      )
      .subscribe(() => {
        this.snackBar.open('Equipment return confirmed.', 'Close', { duration: 3000 });
